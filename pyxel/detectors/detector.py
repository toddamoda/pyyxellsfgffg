...
class Detector:
    ...
    def pixel_read(self) -> PixelRead:
        """Get the pixel information of charge packets within pixel, plus read noises (in electrons)."""
    ...
    @pixel_read.setter
    def pixel_read(self, obj: PixelRead) -> None:
        """Set the pixel_read information for the detector."""
        self.pixel_read.array = obj.array
    ...
    def to_xarray(self) -> "xr.Dataset":
        ...
        for name in ("photon", "charge", "pixel", "pixel_read", "signal", "image"):
            container: Photon | Charge | Pixel | PixelRead | Signal | Image = getattr(
                self, name
            )
