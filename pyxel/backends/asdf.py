...
def to_asdf(filename: Union[str, Path], dct: Mapping[str, Any]) -> None:
    ...
    processed_data: Mapping[str, xr.Dataset] = dct["data"]["processed_data"]
    dct_processed_data: Mapping[str, Mapping] = {
        key: value.to_dict() for key, value in processed_data.items()
    }

    dct["data"]["processed_data"] = dct_processed_data
