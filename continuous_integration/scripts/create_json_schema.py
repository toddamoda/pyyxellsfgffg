...
def generate_all_models() -> Iterator[str]:
    ...
    yield "from typing import Annotated, Any, Literal, Optional, Tuple, Union"
    yield "from collections.abc import Iterator, Mapping, Sequence"
    yield ""
    yield "import click"
    yield "from annotated_types import Ge, Gt, Interval, Le, Len, Lt, MaxLen, MinLen, Unit"
    ...
    yield "    name: str"
    yield "    func: str = field(metadata=schema(pattern=r'^(?!pyxel\\.models\\.)'))"
    yield "    arguments: Optional[Mapping[str, Any]] = None"
    yield "    enabled: bool = True"
    yield ""
    yield ""
